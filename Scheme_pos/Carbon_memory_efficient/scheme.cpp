...
void implementation::make_S(std::vector<S_i> &S){
    for (unsigned int j = 0; j <= 2*p.N_y; j++){
        for (unsigned int i = 0; i <= p.N_e; i++){
...
void implementation::make_B(std::vector<S_i> &S, std::vector<double> &y, unsigned int n, unsigned int i, MTRand_int32 &grand){
    if (p.bd==Drchlt){// boundary condition for carbon terminal
        if(is_odd(n)){
            ...
            y.at(index(2*p.N_y,i)) = p.constant-p.alpha*p.de*static_cast<double>(i)+(1-p.alpha)*(1-p.alpha)*(p.T-p.dt*static_cast<double>((n+1)/2))/(4*p.rho);
            ...
            y.at(index(2*p.N_y,i)) = p.constant-p.alpha*p.de*static_cast<double>(i)+(1-p.alpha)*(1-p.alpha)*(p.T-p.dt*static_cast<double>((n)/2))/(4*p.rho);
            if (i==1){//add Dirichlet boundary condition for e=0 in CN scheme
                ...
                for (unsigned int j=1; j<=2*p.N_y-1; j++){
                    ...
                    m = static_cast<int>(w);
                    w = e/p.de - w;
                    x= ((1-p.gamma)*psi.at(index(j,0))+p.mu)*p.dt;
                    u = floor(x/p.dy);
                    r = static_cast<int>(u);
                    u = x/p.dy - u;
        ...
        if ((!is_odd(n)) && (i == p.N_e-1)){
            for (unsigned int j=0; j<=2*p.N_y; j++){
                y.at(index(j,p.N_e)) = y.at(index(j,p.N_e-1));
            }
        }
    }
}
...
void implementation::make_v_der(std::vector<S_i> &S){
    //V_e and V_y
    for (unsigned int j=1; j<=2*p.N_y-1; j++){
        for (unsigned int i=0; i<=p.N_e-1; i++){
    ...
    for (int j=1; j<=2*p.N_y-1; j++){
        S.at(index(j,p.N_e)).v_y=(S.at(index(j+1,p.N_e)).v-S.at(index(j-1,p.N_e)).v)/(2*p.dy);
    }
    ...
    for (int j=0; j<=2*p.N_y; j++){
        S.at(index(j,p.N_e)).v_e=S.at(index(j,p.N_e-1)).v_e;
    }
...
void implementation::make_psi(std::vector<S_i> &S, unsigned int n){
    for (unsigned int j = 0; j<=2*p.N_y; j++){
        for (unsigned int i=0; i<=p.N_e; i++){
            psi.at(index(j,i)) = max(0,(1 + S.at(index(j,i)).v_e + (1-p.gamma)*S.at(index(j,i)).v_y)/(2*p.rho));
        }
    }
}
...
void implementation::make_coeff(std::vector<S_i> &S, unsigned int n, unsigned int k){
    for (unsigned int j = 1; j<=2*p.N_y-1; j++){
        a.at(j) = p.mu*p.dt/(2*p.dy) + (1-p.gamma)*p.dt*psi.at(index(j,k))/(4*p.dy);
...
void implementation::CN_relaxed_scheme(std::vector<S_i> &S, unsigned int n){
    ...
    bool block1 = true;
    first_block = &block1;
    for (unsigned int i=1; i<=p.N_e; i++){
        unsigned int k = (p.bd==Nmnn) ? p.N_e-i : i;//Neumann goes backward to solve the block matrix while Dirichlet goes forward. k switches the direction based on the choice of ABC
        make_coeff(S,n,k);
        if (p.bd==Drchlt){// Dirichlet  boundary conditions must be set before the step calculations
...
//    cout << "\nn=" << n/2 << "\n";
//    cout << "\n++++++++++++++++++++++++++++++++++++++++++++++++\n";
//    for (unsigned int j=p.N_y-2; j<=p.N_y+2; j++){
//        cout << S.at(index(j,1)).v_e  << ", ";
//    }
//    cout << "\n++++++++++++++++++++++++++++++++++++++++++++++++\n";
//    for (unsigned int j=p.N_y-2; j<=p.N_y+2; j++){
//        cout << S.at(index(j,0)).v_e  << ", ";
//    }
//    cout << "\n++++++++++++++++++++++++++++++++++++++++++++++++\n";
//    for (unsigned int j=p.N_y-2; j<=p.N_y+2; j++){
//        cout << S.at(index(j,1)).v_y  << ", ";
//    }
//    cout << "\n++++++++++++++++++++++++++++++++++++++++++++++++\n";
//    for (unsigned int j=p.N_y-2; j<=p.N_y+2; j++){
//        cout << S.at(index(j,0)).v_y << ", ";
//    }
//    cout << "\n->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->\n";
    reset_y1();
...
void implementation::make_tau(std::vector<S_i> &S, std::vector<S_i> &tau){
    ...
    for(unsigned int i=0; i<S.size(); i++){
        D0.y = S.at(i).y;
        D0.e = S.at(i).e;
        D0.v = (1-p.gamma)*S.at(i).v_y+(1/p.rho-1)*max(0,1+S.at(i).v_e+(1-p.gamma)*S.at(i).v_y);
        D0.v_e = 0;
        D0.v_y = 0;
        tau.at(i).v=D0.v;
    }
...
void implementation::write2file(vector<S_i> &S, const bool &file, std::string file_name){
    if (file){
        ...
        for (unsigned int j=p.N_y/2; j<=3*p.N_y/2; j++){
            for (unsigned int i=0; i<=p.N_e/2; i++){
...
implementation::implementation(const param &p_input){
    ...
    if (log_file.is_open()){
        ...
        log_file  << "\nNumber of intervals for e: " << p.N_e;
    ...
    for (unsigned int j=0; j<=2*p.N_y; j++){
        ...
        for (unsigned int i=0; i<=p.N_e; i++){//psi should be saved for the next step, y1 saves the value function at current until we are done with the value function oin previous step
...
void make_latex::write_file(){
    if (latex_file.is_open()){
        ...
        latex_file << "$\\gamma=" << p.gamma << "$, $\\alpha=" << p.alpha << "$, $\\beta=1$, $\\pi=q(1-q)$, $\\eta(q)=\\lambda(q)=q$, $T=" << p.T << "$, $\\mu=" << p.mu << "$ and $\\varrho=" << p.rho << "$ ($a=" << 1/(2*(1-p.rho))<< "$),  bounded domain for $(t,y,e)$ is $[0," << p.T << "]\\times[" << -p.L_y << ", " << p.L_y << "]\\times[0," << p.L_e << "]$ with ";
        if (p.bd==Nmnn){
